...
```
phone video  ->  frame extraction  ->  AMB3R reconstruction  ->  semantic labelling  ->  point clouds + transforms.json
```
---
...
| **GPU** | NVIDIA with 8 GB+ VRAM | Tested on RTX 5070, 8 GB |
| **NVIDIA Driver** | 570+ | Required for RTX 50-series (Blackwell). Check with `nvidia-smi` |
| **Docker** | 20.10+ | Check with `docker --version` |
| **NVIDIA Container Toolkit** | Any recent | Lets Docker access your GPU. See Step 2 below. |
| **RAM** | 16 GB+ | |
| **Disk** | ~13 GB | ~11 GB Docker image (incl. segmentation model) + ~1 GB AMB3R weights |
---
...
> ```
> Building from source compiles `flash-attn` and `torch-scatter` for your GPU architecture (Blackwell / sm_120) and bakes in the segmentation model, so it takes **~30–60 minutes**. The pre-built image above already contains all of this.
...
This downloads `amb3r.pt` (~1 GB) into `checkpoints/`. Only needed once. The semantic-segmentation model is baked into the Docker image during Step 4, so it does **not** need to be downloaded.
...
Run this once to give display access to Docker:
```bash
xhost +local:docker
```
...
├── labels.json           ← class -> colour + point-count summary
└── frames/               ← extracted video frames
```

Semantic labelling runs by default and the viewer opens the labelled cloud. Use `--no-semantic` to skip it and produce only the geometric reconstruction.
...
--max_frames   Max frames to use        (default: 150  | on 8 GB GPU max value before OOM ~18)
--conf         Confidence threshold     (default: 0.5  | range: 0.0–1.0)
--device       cuda or cpu              (default: cuda)
--checkpoint   Path to weights file     (default: checkpoints/amb3r.pt)
--no-semantic  Skip the semantic labelling stage
--no-viewer    Skip the interactive 3D viewer (headless / server use)
```
...
semantic.py          — A Mask2Former model segments each frame into classes;
                       ...
                       Writes scene_semantic.ply + labels.json.  (--no-semantic
                       skips this stage.)
```
...
- Make sure your NVIDIA driver is up to date (`nvidia-smi` should show driver 570+ for RTX 50-series)
